# Front-End Development Starter Code

This template provides a minimal setup to get started on CA2. It has Vite + React + ReactRouter + Tailwind CSS + ShadCN installed.

## Instructions:

1. Make a copy of this repo
2. Give the copy the name of your application e.g. `ca2-Medical CLinic`
3. Change the `name` property in `package.json` to your application name
4. `npm install`
5. `npx shadcn@latest init` // you only need to do this once
    - Select a base color
6. `npm run dev`



In this REPO there is a full CRUD for a medical clinic without loggin in you are not able to go to any endpoint that isn't the basic view.

To register an account u can go to 